...
Fifty-three of those run today, on the Shopify, SFCC, Gap Inc. and JSON-LD
engines plus two bespoke adapters. The rest wait on two engines still to be built
(Hybris, Magento) — [docs/SOURCES.md](docs/SOURCES.md#not-built-yet) lists exactly what is
and is not shipped, so `npm run health` showing 56 sources rather than 101 is not
a surprise.
...
| `npm test` / `test:coverage` | 574 tests, no network |
